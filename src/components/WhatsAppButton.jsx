...
export default function WhatsAppButton() {
  ...
  const message = 'Olá! Gostaria de obter mais informações sobre os cursos da NextDrive Academy.'
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="bg-white rounded-2xl shadow-2xl border border-slate-100 p-5 w-72"
          >
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <p className="font-semibold text-slate-900 text-sm">NextDrive Academy</p>
                <p className="text-xs text-green-500 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 bg-green-500 rounded-full inline-block"></span>
                  Online agora
                </p>
              </div>
              <button onClick={() => setIsOpen(false)} className="ml-auto text-slate-400 hover:text-slate-600">
                <X size={16} />
              </button>
            </div>
            <div className="bg-slate-50 rounded-xl p-3 mb-4 text-sm text-slate-700 leading-relaxed">
              Olá! 👋 Como posso ajudar? Clique abaixo para falar diretamente connosco no WhatsApp.
            </div>
            <a
